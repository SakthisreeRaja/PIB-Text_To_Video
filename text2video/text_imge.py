...
def create_video_from_images(image_paths, total_duration, output_file="input_video.mp4"):
    ...
    base_width, base_height = 1200, 675  
    try:
...
if __name__ == "__main__":
    ...
    if os.path.exists(DURATION_FILE):
        try:
            ...
            print(f" Warning: Could not read duration from {DURATION_FILE}. Falling back to default {target_duration}s. Error: {e}")
    else:
        print(f" Warning: {DURATION_FILE} not found. Falling back to default {target_duration}s. ENSURE Phase 1 ran first.")
    ...
    if not image_paths:
        ...
        print(" Primary directory 'assets' empty or not found. Checking backup: backup_assets...")
